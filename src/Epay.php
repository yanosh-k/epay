...
class Epay
{
    ...
    public static $defaultSettings = array(
        'submitURL'     => 'https://demo.epay.bg/', // the url where the transaction data will be submited
        'clientNumber'  => null,
        'clientSecret'  => null,
        'clientEmail'   => null,
     ...
     * @return string
     */
    public function createForm($formId = 'epay-payment-form', $formName = "")
    {
        $form = "<form action='{$this->settings['submitURL']}' method='POST' id='{$formId}' name='{$formName}'>";
        foreach ($this->getDataArray() as $field => $value) {
    ...
    /**
     * Extracts the parameters from a notification item. This notification
     * indicates a change in the transaction status
     * 
     * @param string $line
     * @return array
     */
    ...
    /**
     * Generate the response body that should be sent from our payment notification
     * endpoint
     * 
     * @param array $params
     * @return string
